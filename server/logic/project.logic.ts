...
/**
 * Project with hierarchy and aggregate progress
 */
export interface ProjectWithHierarchy extends projectsRepo.CampaignProject {
	children?: ProjectWithHierarchy[];
	aggregate_progress?: {
 ...
 */
export function buildProjectTree(projects: projectsRepo.CampaignProject[]): ProjectWithHierarchy[] {
	const projectMap = new Map<number, ProjectWithHierarchy>();
	const rootProjects: ProjectWithHierarchy[] = [];
 ...
 */
export async function calculateAggregateProgress(project_id: number): Promise<{
	total_goal_points: number;
	total_current_points: number;
	total_percentage: number;
}> {
...
export async function updateProjectProgress(
	...
	user_id: number,
	new_points: number,
	is_increment: boolean = false,
	notes?: string
): Promise<projectsRepo.CampaignProject | null> {
	// Get current project
	const project = await projectsRepo.findById(project_id);
	if (!project) {
		return null;
 ...
 */
export async function checkAutoComplete(project_id: number): Promise<boolean> {
	const project = await projectsRepo.findById(project_id);
	if (!project) {
		return false;
	}
