...
export class Utils {
	...
	static export = (elementIDs: string[], name: string, data: unknown, ext: string, format: 'image' | 'pdf' | 'json') => {
		switch (format) {
			case 'image':
				Utils.exportImage(elementIDs, name);
				break;
			case 'json':
				Utils.saveFile(data, name, ext);
				break;
		}
	};
