...
export const SourcebooksModal = (props: Props) => {
	...
	// Built-in homebrew sourcebooks (like Draachenmar) that come from officialSourcebooks
	const builtInHomebrewSourcebooks = props.officialSourcebooks.filter(s => s.type === SourcebookType.Homebrew);
			...
			case SourcebookType.Homebrew:
				return (
					<>
						...
						<Space orientation='vertical' style={{ width: '100%' }}>
							{/* Built-in homebrew sourcebooks (like Draachenmar) - read-only */}
							{
								Collections.sort(builtInHomebrewSourcebooks, sb => sb.name).map(s => (
									<SelectablePanel key={s.id}>
										<SourcebookEditorPanel
											sourcebook={s}
											sourcebooks={[ ...props.officialSourcebooks, ...homebrewSourcebooks ]}
							...
							{/* User-created homebrew sourcebooks */}
							{
							...
							{
								(builtInHomebrewSourcebooks.length === 0 && homebrewSourcebooks.length === 0) ?
									<Empty />
									: null
							}
						</Space>
